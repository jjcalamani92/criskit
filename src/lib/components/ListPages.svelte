    ...
    <div class="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
      ...
      <!-- <a href="{$page.url.pathname}/pages/{_id}" class="group"> -->
        <div  class="group relative">
          <input type="checkbox" class="h-5 w-5 rounded border-gray-400 text-indigo-600 focus:ring-indigo-500 absolute  top-2 left-2 z-10 opacity-0 group-hover:opacity-100 transition ease-in-out delay-150"/>
        ...
        <a href="{$page.url.pathname}/pages/{_id}" class="mt-4 text-sm text-gray-700">{name}</a>
