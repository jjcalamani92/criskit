...
/**
 * @param {{ i: string; name: string; parentId: string; description: string; siteId: string; uid: string; typeCategory: string; type: string; thumbnailUrl: string; paths: any; }} body
 */
export async function addCategory(body) {
	const { i, name, parentId, description, siteId, uid, typeCategory, type, thumbnailUrl, paths } =
		body;
...
export async function updateCategory(body) {
	const { id, name, parentId, description, siteId, uid, typeCategory, type, thumbnailUrl, i } =
		body;
	let ids0;
	let ids1;
	const client = await clientPromise0;
	...
	const category = await db
		...
		.findOne({ _id: new ObjectId(id) }, { projection: { _id: 0, parentId: 1, slug: 1 } });
	...
	try {
		...
		await db.collection(`categories${i}`).updateOne(
			...
			{
				arrayFilters: [
					{ element: { $eq: category?.slug } },
					{ 'ele.slug': { $eq: category?.slug } }
				]
			}
		);
